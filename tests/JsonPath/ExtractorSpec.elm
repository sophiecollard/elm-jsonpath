    ...
    describe "The Extractor module's"
        [ describe "run method"
            ...
            , describe "should extract the elements in a path comprising a slice selector"
                [ test "$.store.book[1:3]" <|
                    \_ ->
                        equal (JsonPath.Extractor.run "$.store.book[1:3]" True sampleJson)
                            (Ok (Json.Encode.list identity [ book1, book2 ]))
                , test "$.store.book[2:]" <|
                    \_ ->
                        equal (JsonPath.Extractor.run "$.store.book[2:]" True sampleJson)
                            (Ok (Json.Encode.list identity [ book2, book3 ]))
                , test "$.store.book[-3:]" <|
                    \_ ->
                        equal (JsonPath.Extractor.run "$.store.book[-3:]" True sampleJson)
                ...
                , test "$.store.book[0:4:2]" <|
                    \_ ->
                        equal (JsonPath.Extractor.run "$.store.book[0:4:2]" True sampleJson)
                ...
                , test "$.store.book[::-1]" <|
                    \_ ->
                        equal (JsonPath.Extractor.run "$.store.book[::-1]" True sampleJson)
                            (Ok (Json.Encode.list identity [ book3, book2, book1, book0 ]))
                , test "$.store.book[::-2].author" <|
                    \_ ->
                        equal (JsonPath.Extractor.run "$.store.book[::-2].author" True sampleJson)
            ...
            , describe "should extract the elements in a path comprising an index selector with multiple values"
                ...
                , test "$.store.book[-2,-1].author" <|
                    \_ ->
                        equal (JsonPath.Extractor.run "$.store.book[-2,-1].title" True sampleJson)
            ...
            , describe "should extract the elements in a path comprising a key selector with multiple values"
                [ test "$.store.book[1][author,title]" <|
                    \_ ->
                        equal (JsonPath.Extractor.run "$.store.book[1][author,title]" True sampleJson)
                            (Ok (Json.Encode.list Json.Encode.string [ "Evelyn Waugh", "Sword of Honour" ]))
                , test "$.store.book[-2,-1][author,title]" <|
                    \_ ->
                        equal (JsonPath.Extractor.run "$.store.book[-2,-1][author,title]" True sampleJson)
                            (Ok
                                (Json.Encode.list Json.Encode.string
                                    [ "Herman Melville"
                                    , "Moby Dick"
            ...
            , describe "should correctly report an 'index not found' error"
                [ test "$.store.book[5].author" <|
                    \_ ->
                        equal (JsonPath.Extractor.run "$.store.book[5].author" True sampleJson)
            ...
            , describe "should correctly report a 'key not found' error"
                ...
                , test "$.store.book[0][author,publisher]" <|
                    \_ ->
                        equal (JsonPath.Extractor.run "$.store.book[0][author,publisher]" True sampleJson)
            ...
            , describe "should correctly report a 'not a JSON array' error"
                [ test "$.store[1]" <|
                    \_ ->
                        equal (JsonPath.Extractor.run "$.store[1]" True sampleJson)
            ...
            , describe "should correctly report a 'not a JSON array nor an object' error"
                [ test "$.store.book[1].author[*]" <|
                    \_ ->
                        equal (JsonPath.Extractor.run "$.store.book[1].author[*]" True sampleJson)
